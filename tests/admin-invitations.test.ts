...
test("招待一覧の絞り込み・クリア・ページ移動はスクロール位置を維持する", async () => {
  ...
  assert.equal(paginationSource.match(/href=\{buildAdminInvitationHref\([\s\S]*?scroll=\{false\}/g)?.length, 4);
  assert.equal(source.match(/<AdminInvitationPagination query=\{invitationQuery\} pagination=\{pagination\} \/>/g)?.length, 2);
});

test("招待一覧は件数サマリーの直下と一覧末尾にページングを表示する", async () => {
  const source = await readFile("src/app/admin/page.tsx", "utf8");

  assert.match(
    source,
    /全\{pagination\.totalCount\}件中[\s\S]*?<AdminInvitationPagination query=\{invitationQuery\} pagination=\{pagination\} \/>[\s\S]*?hidden overflow-x-auto/
  );
});
