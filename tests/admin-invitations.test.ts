...
test("共有名は前後空白を除去・NFKC正規化・長さ制限して大文字小文字を区別せず部分一致検索する", () => {
  ...
  assert.deepEqual(buildAdminInvitationWhere(query({ search: "林家" }), now), {
    household: { name: { contains: "林家", mode: "insensitive" } }
  });
