...
test("更新Actionは未来日・Household所属・管理外制御とrevision同一トランザクションを維持する", () => {
  ...
  assert.match(actions, /recordRedirect\(result\.data\.hamsterId, "futureTime"\)/);
...
test("共通タイムラインは共通ページングを使用し、検索条件とスクロール位置を維持する", () => {
  ...
  assert.equal(page.match(/<PaginationLayout/g)?.length, 1);
  assert.match(page, /<RecordTimeline[\s\S]*?<PaginationLayout/);
  assert.match(page, /ariaLabel="記録一覧のページ移動"/);
  assert.match(page, /visibleCount=\{data\.records\.length\}/);
  assert.match(page, /buildHref=\{buildRecordsPageHref\}/);
  assert.match(page, /scroll=\{false\}/);
  assert.match(page, /preserveScroll/);
  assert.match(page, /const buildRecordsPageHref = \(page: number\) => recordsHref\(\{ \.\.\.currentFilters, page \}\)/);
  for (const filter of ["hamsterId", "type", "from", "to", "keyword", "favoriteOnly"]) {
    assert.match(page, new RegExp(`if \\(filters\\.${filter}`));
  }
