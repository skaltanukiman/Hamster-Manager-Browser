...
test("思い出記録はタイトル・内容を必須にし、自由タグを正規化する", () => {
  ...
  assert.deepEqual(withoutSaving.tags, ["かわいい", "天然", "くさい"]);
...
test("保存対象の思い出タグをHousehold単位の個別行へ正規化する", () => {
  assert.deepEqual(buildSavedMemoryTagRows("household-1", "user-1", ["かわいい", "天然", "クサイ"]), [
    { householdId: "household-1", createdByUserId: "user-1", name: "かわいい", normalizedName: "かわいい" },
    { householdId: "household-1", createdByUserId: "user-1", name: "天然", normalizedName: "天然" },
    { householdId: "household-1", createdByUserId: "user-1", name: "クサイ", normalizedName: "くさい" }
  ]);
});
