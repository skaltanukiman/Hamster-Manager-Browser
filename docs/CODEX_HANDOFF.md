...
- `src/app/api/realtime/household/route.ts` と `src/components/realtime-refresh-listener.tsx` で Household 単位の SSE 同期を行う。
- DB 更新後の Server Action は `notifyHouseholdChange()` を呼び、別タブ・別ユーザー画面へ変更を通知する。
- `form[data-dirty-watch]` に未保存変更がある場合は自動 `router.refresh()` せず、右下通知から手動再読み込みする。
- 現状の通知バスは app プロセス内メモリ。Docker Compose の app 1プロセス運用向けで、複数 app インスタンスにする場合は PostgreSQL LISTEN/NOTIFY など外部 pub/sub へ差し替える。
