...
- メンバー参加解除は `OWNER` のみ。自分自身と最後の OWNER は解除不可。
...
- SSE が届かない場合でも、クライアントは 4 秒おきに revision API で `updatedAt` を確認して追従する。
- フォーム送信時は `realtimeActorId` を hidden input / FormData に差し込み、自分自身や同一タブ由来の更新は自動再読み込み対象から外す。
...
- `households`
  - 共有単位。`updatedAt` はリアルタイム更新検知の revision としても使う。
- `household_members`
  - `OWNER` / `ADMIN` / `MEMBER`
