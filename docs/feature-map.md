...
- **健康記録の任意時刻:** `RecordTimeInput` が「時間も記録する」選択時だけ `recordTime` を入力・編集し、`src/lib/record-time.ts` が `HH:mm` と0〜1439分を相互変換する。`HamsterRecord.recordTimeMinutes` は健康記録だけが使用する任意の `SmallInt` で、migration `20260717120000_add_health_record_time` が範囲制約と並び順用索引を追加する。カードは日付、任意時刻、登録者の順に表示し、同日内は時刻ありの降順、時刻なし、作成日時、IDの順で取得する。
