...
| PWA メタデータ | `src/app/manifest.ts`, `src/app/layout.tsx`, `src/app/apple-icon.png`, `public/icons/pwa-192.png`, `public/icons/pwa-512.png`, `public/icons/pwa-maskable-512.png` | App Router の標準 Metadata API でインストール情報、テーマ色、通常・maskable・Apple用アイコンを配信する。Service Worker、オフライン機能、プッシュ通知は持たない。`tests/manifest.test.ts` でManifestの主要項目を検証する。 |
