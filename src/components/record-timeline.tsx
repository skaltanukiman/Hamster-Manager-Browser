...
function RecordEditForm({ record, hamsterId, today }: { record: RecordItem; hamsterId: string; today: string }) {
...
export function RecordTimeline({ records, hamsterId, hamsterIsActive, canEdit, today }: { records: RecordItem[]; hamsterId: string; hamsterIsActive: boolean; canEdit: boolean; today: string }) {
        ...
        {records.map((record) => {
          ...
          return (
            <article key={record.id} className="relative ml-9 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:ml-12 sm:p-5">
              ...
              {editable ? <details className="mt-4"><summary className="inline-flex cursor-pointer items-center gap-1 text-sm font-semibold text-moss"><Pencil className="h-4 w-4" aria-hidden />編集フォームを開く</summary><RecordEditForm record={record} hamsterId={hamsterId} today={today} /></details> : !canEdit ? null : <p className="mt-4 text-xs text-amber-700">管理外のため、この健康・通院記録は閲覧のみです。</p>}
            </article>
          );
        })}
      </div>
