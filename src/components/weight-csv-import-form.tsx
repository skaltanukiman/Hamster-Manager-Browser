...
export function WeightCsvImportForm({ mode }: { mode: "app" | "gas" }) {
  const action = mode === "app" ? importAppWeightRecordsCsv : importGasWeightRecordsCsv;
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="text-base font-bold text-ink">{isAppMode ? "アプリ版CSVを一括編集" : "GAS版CSVをインポート"}</h3>
      <form action={formAction} className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
        ...
        >
          ...
          {isPending ? "インポート中..." : isAppMode ? "一括編集する" : "インポート"}
        </button>
      </form>
      ...
      {state.hasResult ? (
        ...
        >
          ...
          <dl className={`mt-3 grid gap-2 ${isAppMode ? "sm:grid-cols-4" : "sm:grid-cols-3"}`}>
            <div className="rounded-md bg-white/70 px-3 py-2">
              <dt className="text-xs">{isAppMode ? "新規登録" : "登録成功"}</dt>
              <dd className="text-lg font-bold">{state.successCount} 件</dd>
            </div>
            {isAppMode ? (
              <div className="rounded-md bg-white/70 px-3 py-2">
                <dt className="text-xs">更新</dt>
                <dd className="text-lg font-bold">{state.updatedCount} 件</dd>
              </div>
            ) : null}
            <div className="rounded-md bg-white/70 px-3 py-2">
