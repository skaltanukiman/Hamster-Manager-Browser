      ...
      {data.hamsters.length === 0 ? (
        ...
        <>
          ...
          <section className="grid gap-4">
            ...
            {data.pagination.totalPages > 1 ? <nav className="flex flex-wrap items-center justify-center gap-2" aria-label="記録ページ移動"><Link aria-label="最初のページ" href={recordsHref({ ...currentFilters, page: 1 })} className="grid h-10 w-10 place-items-center rounded-md border border-slate-200 bg-white"><ChevronsLeft className="h-4 w-4" /></Link><Link aria-label="前のページ" href={recordsHref({ ...currentFilters, page: Math.max(data.pagination.currentPage - 1, 1) })} className="grid h-10 w-10 place-items-center rounded-md border border-slate-200 bg-white"><ChevronLeft className="h-4 w-4" /></Link><span className="px-2 text-sm font-semibold text-slate-700">{data.pagination.currentPage} / {data.pagination.totalPages}</span><Link aria-label="次のページ" href={recordsHref({ ...currentFilters, page: Math.min(data.pagination.currentPage + 1, data.pagination.totalPages) })} className="grid h-10 w-10 place-items-center rounded-md border border-slate-200 bg-white"><ChevronRight className="h-4 w-4" /></Link><Link aria-label="最後のページ" href={recordsHref({ ...currentFilters, page: data.pagination.totalPages })} className="grid h-10 w-10 place-items-center rounded-md border border-slate-200 bg-white"><ChevronsRight className="h-4 w-4" /></Link></nav> : null}
          </section>
        </>
      )}
    </main>
