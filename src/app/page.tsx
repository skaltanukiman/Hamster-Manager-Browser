...
import { CalendarDays, ClipboardCheck, Plus, Scale } from "lucide-react";
...
export default async function DashboardPage() {
  const hamsters = await getDashboardData();
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">登録済みハムスターの最新状態</p>
        </div>
        <Link
          href="/hamsters"
            ...
            return (
              <article key={hamster.id} className="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="text-lg font-bold text-ink">{hamster.name}</h3>
                    {hamster.memo ? <p className="mt-1 line-clamp-2 text-sm text-slate-500">{hamster.memo}</p> : null}
                  </div>
                  <span className="rounded-md bg-straw/40 px-2 py-1 text-xs font-semibold text-slate-700">
                    管理中
                  </span>
                </div>
