...
- 体調記録は総合状態、食欲、活動量、便、尿、複数症状、メモを保存。「いつも通り」は5つの状態だけを正常値へ設定し、症状とメモは消去しない
- 通院記録は通院理由だけを内容必須とし、病院名、診断、検査、治療、処方薬、投薬方法、次回予定日、診察費、メモは任意。診察費は0円以上の整数
- 思い出記録はタイトル、内容、自由タグ、お気に入り、写真1枚を保存。タグは「、」またはカンマ区切りで最大20件。「入力したタグを保存して再利用する」を選ぶと、各タグをHousehold共通の候補として個別保存し、次回以降は候補ボタンから入力可能
- JPEG / PNG / WebP の思い出写真を2MB以内で受け付け、512px正方形のWebPへ変換。認証と現在のHousehold所属を確認する `/api/records/[id]/image` だけから配信
- 3種類を記録日の新しい順で同じタイムラインへ表示し、同日内は作成日時とIDで安定ソート
- 種類、期間、キーワード、お気に入りでDB側絞り込み。通常キーワードはタイトル、メモ、症状、通院内容、薬、思い出本文を、`#タグ` は思い出タグだけを対象とし、20件ずつページング
...
- 思い出削除、画像差し替え、ハムスター削除では不要になった画像を削除。DB更新後のファイル削除失敗は警告ログへ残す
...
### `hamster_records` / 種類別詳細 / `memory_record_images`

- `hamster_records`: `hamsterId`、`recordType`、`recordDate`、`title`、`memo`、`searchText`、`createdByUserId`、作成・更新日時を持つ共通タイムライン親
- `health_record_details`: 総合状態、食欲、活動量、便、尿、複数症状
- `medical_visit_details`: 病院名、理由、診断、検査、治療、薬、投薬方法、次回予定日、診察費
- `memory_record_details`: 自由タグとお気に入り
- `memory_record_images`: 思い出画像のサーバー生成ファイル名と表示順。初回実装は1記録1枚だが、別テーブルのため複数画像へ拡張可能
- `saved_memory_tags`: Household内で再利用する思い出タグ。かな表記・全半角等を正規化した名前で重複を防止

親記録と詳細・画像メタデータはCascadeで削除します。`searchText` は入力項目から生成した検索専用テキストで、PostgreSQL `pg_trgm` のGINインデックスを使用します。別HouseholdのハムスターIDを指定した取得・更新はサーバー側の所属条件で拒否します。
...
プロフィール画像列は migration `20260712090000_add_hamster_profile_image`、健康・通院・思い出記録は migration `20260715120000_add_hamster_records`、保存済み思い出タグは migration `20260716160000_add_saved_memory_tags` で追加されます。appコンテナ起動時にも `prisma migrate deploy` が実行されるため、更新時はログで適用成功を確認してください。
