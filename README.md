...
- ハムスター一覧、衛生管理メモ、体重履歴の保存ボタンは、編集が入るまで非活性
- 同じ Household のデータが他の画面で更新された場合は SSE 通知または 4 秒ごとの更新時刻チェックで検知し、未保存の入力がなければ画面を自動再取得
...
- 掃除項目のチェックボックスは変更時に自動保存し、同じ共有を開いている画面へ通知
...
共有の単位です。新規ログインユーザーに Household がない場合は、`{ユーザー名}のハムスター管理` という個人用 Household を自動作成します。

`updatedAt` は Household 単位のリアルタイム更新検知の revision としても使います。
...
招待リンクの管理テーブルです。招待トークンは画面表示時のみ平文で扱い、DB には SHA-256 ハッシュを保存します。
...
OWNER は `/settings/members` からメンバーの共有参加を解除できます。自分自身の共有解除と最後の OWNER の解除はできません。
